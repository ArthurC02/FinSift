...
acctfinder and callfinder each define print_summary_rows and write_summary_csv.
...
def test_F6_acctfinder_print_summary_rows(capsys):
    acctfinder.print_summary_rows([FIN_ROW])
...
def test_F6_callfinder_print_summary_rows(capsys):
    ...
    callfinder.print_summary_rows([CALL_RATIO_ROW, CALL_VALUE_ROW])
    out = capsys.readouterr().out
...
def test_F6_acctfinder_write_summary_csv(tmp_path):
    out_path = acctfinder.write_summary_csv(tmp_path, [FIN_ROW])
...
def test_F6_callfinder_write_summary_csv(tmp_path):
    out_path = callfinder.write_summary_csv(tmp_path, [CALL_RATIO_ROW, CALL_VALUE_ROW])
...
def test_E4_combined_csv_fallback_row_keeps_both_ROA_and_ROE(tmp_path):
    ...
    disagreed."""
    out_path = acctfinder.write_combined_csv(tmp_path, {}, [FALLBACK_RATIO_ROW], used_fallback=True)
    with open(out_path, encoding="utf-8-sig", newline="") as f:
...
def test_E4b_non_fallback_rows_still_use_format_pct(tmp_path):
    ...
    out_path = acctfinder.write_combined_csv(tmp_path, {}, [disclosed], used_fallback=False)
    with open(out_path, encoding="utf-8-sig", newline="") as f:
        header, row = list(csv.reader(f))
    values = dict(zip(header, row))
...
def test_C1_full_coverage_says_nothing():
    assert acctfinder.summary_coverage_warning(_rows(18, 0)) is None
...
def test_C2_a_few_na_rows_are_ordinary():
    ...
    assert acctfinder.summary_coverage_warning(_rows(16, 2)) is None


def test_C3_exactly_half_na_is_still_below_the_line():
    # BVT: the threshold fires on MORE than half, not on half.
    assert acctfinder.summary_coverage_warning(_rows(9, 9)) is None
    assert acctfinder.summary_coverage_warning(_rows(8, 10)) is not None


def test_C4_a_wholesale_failed_read_names_what_is_missing():
    warning = acctfinder.summary_coverage_warning(_rows(2, 16), folder="fin_q4")
...
def test_C5_no_rows_is_not_a_coverage_problem():
    ...
    assert acctfinder.summary_coverage_warning([]) is None
