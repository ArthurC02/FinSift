...
def test_F1_enough_coded_rows_is_a_fin_report(tmp_path):
    ...
    assert cli.classify_folder(folder, CODES)[0] == "fin_report"
...
def test_F2_finsum_marker_and_few_files_routes_to_the_summary_extractor(tmp_path):
    ...
    assert cli.classify_folder(folder, CODES)[0] == "fin_report_summary"


def test_F3_finsum_marker_but_too_many_files_is_a_full_filing(tmp_path):
    files = {f"{i:03d}.md": "" for i in range(cli._FINSUM_MAX_FILES + 1)}
...
def test_F4_con_call_markers_without_coded_rows(tmp_path):
    ...
    assert cli.classify_folder(folder, CODES)[0] == "con_call"


def test_F5_neither_signal(tmp_path):
    folder = make_folder(tmp_path, "f5", {"001.md": "封面\n"})
    assert cli.classify_folder(folder, CODES)[0] is None
...
def test_code_hit_threshold_boundary(tmp_path, hits, expected):
    ...
    assert cli.classify_folder(folder, CODES)[0] == expected
...
def test_classify_folder_scans_subdirectories_like_every_extractor(tmp_path):
    ...
    subdirectory was classified None and skipped by cli, while fin
...
# --------------------------------------------------------------------------
# §5.6 disclosures.resolve_period - Decision Table (5)
# --------------------------------------------------------------------------
...
def test_Q1_no_request_takes_the_newest():
    assert disclosures.resolve_period(LINKS) == (114, 12, "u12", True)


def test_Q2_requested_month_is_published():
    assert disclosures.resolve_period(LINKS, 114, 9) == (114, 9, "u9", True)
...
def test_Q3_falls_back_to_the_newest_earlier_month():
    ...
    assert disclosures.resolve_period(LINKS, 114, 11) == (114, 9, "u9", False)


def test_Q4_fallback_never_jumps_forward_when_an_earlier_month_exists():
    assert disclosures.resolve_period(LINKS, 115, 3) == (114, 12, "u12", False)
...
def test_Q5_request_older_than_everything_falls_forward_to_the_oldest():
    ...
    """
    assert disclosures.resolve_period(LINKS, 113, 1) == (114, 6, "u6", False)


def test_the_fallback_note_explains_the_right_direction():
    forward = disclosures._result("逾期放款總額", 114, 6, False, "u", "千元", {}, (113, 1))
    assert "no longer in the published dataset" in forward["note"]
    assert "used 114年6月 instead" in forward["note"]

    backward = disclosures._result("逾期放款總額", 114, 9, False, "u", "千元", {}, (114, 11))
...
def test_M1_disjoint_terms_come_back_in_MERGED_TERM_ORDER():
    ...
    con = [row("NIM")]
    assert [r[0] for r in cli.merge_fin_and_con_rows(fin, con)] == ["總資產", "稅後淨利", "NIM"]


def test_M2_terms_outside_the_order_list_are_appended():
    fin = [row("總資產"), row("未知項目")]
    assert [r[0] for r in cli.merge_fin_and_con_rows(fin, [])] == ["總資產", "未知項目"]
...
def test_M3_a_term_present_on_both_sides_is_emitted_once():
    ...
    fin = [row("總資產", 100)]
    con = [row("總資產", 200)]
    merged = cli.merge_fin_and_con_rows(fin, con)
...
def test_M4_one_empty_side_passes_the_other_through():
    ...
    assert [r[0] for r in cli.merge_fin_and_con_rows([], con)] == ["NIM", "存放利差"]
...
def test_E6_detect_bank_reads_the_same_file_window_as_detect_industry(tmp_path):
    ...
    detected its industry fine and failed on the bank, and cli skipped
    the whole folder with 'Couldn't auto-detect the bank'."""
    folder = make_folder(tmp_path, "late_name", {
        "001.md": "封面\n",
        "002.md": "玉山商業銀行股份有限公司\n",
    })
    assert fin.detect_bank(folder) == "玉山"
    assert fin.detect_industry_category(folder) == "金融業"
...
def test_B1_a_single_named_bank_is_detected(tmp_path):
    ...
    assert fin.bank_candidates(folder) == ["玉山"]
    assert fin.detect_bank(folder) == "玉山"
...
def test_B2_two_named_banks_refuse_to_resolve(tmp_path):
    ...
    assert fin.bank_candidates(folder) == ["國泰", "玉山"]
    assert fin.detect_bank(folder) is None


def test_B3_no_bank_name_at_all_is_still_none(tmp_path):
    folder = make_folder(tmp_path, "b3", {"001.md": "某控股股份有限公司\n"})
    assert fin.bank_candidates(folder) == []
    assert fin.detect_bank(folder) is None
...
def test_B4_the_message_separates_ambiguous_from_unsupported(tmp_path):
    ...
    assert "Several banks" in fin.bank_detection_message(ambiguous)
    assert "玉山" in fin.bank_detection_message(ambiguous)
    assert "中信" in fin.bank_detection_message(ambiguous)
    assert "Couldn't auto-detect" in fin.bank_detection_message(unknown)
...
def test_I1_an_insurance_filing_is_refused_not_relabelled(tmp_path):
    ...
    assert fin.detect_industry_category(folder) == "保險業"
    ...
    with pytest.raises(ValueError) as excinfo:
        fin.collect_summary_rows(folder, "國泰")
...
def test_I2_a_bank_filing_still_resolves_its_layout(tmp_path):
    ...
    assert fin.detect_industry_category(folder) == "金融業"
    rows = fin.collect_summary_rows(folder, "玉山")
...
def test_I3_an_unidentifiable_industry_is_refused_too(tmp_path):
    ...
    assert fin.detect_industry_category(folder) is None
    with pytest.raises(ValueError):
        fin.collect_summary_rows(folder, "玉山")
    assert fin.collect_summary_rows(folder, "玉山", industry="金融業") is not None
...
def test_I4_the_finsum_wrapper_carries_the_industry_through(tmp_path):
    ...
    with pytest.raises(ValueError):
        fin.collect_summary_rows_finsum(folder, "國泰")
...
def test_I5_every_supported_industry_maps_to_a_real_layout():
    ...
    assert set(fin.INDUSTRY_SUMMARY_LAYOUTS) <= set(fin.INDUSTRY_CODING_FILES)
    for industry, layout in fin.INDUSTRY_SUMMARY_LAYOUTS.items():
...
def test_P1_every_derived_view_matches_its_profile():
    ...
    assert fin.BANKS == list(fin.BANK_PROFILES)
    for name, profile in fin.BANK_PROFILES.items():
...
def test_P5_industry_scoping_is_what_separates_a_group_s_bank_from_its_insurer(tmp_path):
    ...
    assert fin.bank_candidates(bank) == ["國泰"]
    assert fin.bank_candidates(life) == []
    ...
    assert fin.bank_candidates(life, industry=None) == []
    assert fin.bank_candidates(life, industry="金融業") == ["國泰"]
...
def test_P6_an_earnings_call_deck_still_resolves_with_no_industry(tmp_path):
    ...
    assert fin.detect_industry_category(deck) is None
    assert fin.detect_bank(deck) == "玉山"
...
def test_P6c_no_alias_makes_another_entity_a_candidate(tmp_path):
    ...
    for name, profile in fin.BANK_PROFILES.items():
        ...
        assert fin.bank_candidates(folder) == [name]
...
def test_P7_stating_an_empty_override_is_the_same_as_omitting_the_entity():
    ...
    them with .get(bank, {}) (fin:785 and collect_summary_rows), so
    absent and empty behave identically - this pins WHICH entities actually
    carry an override, so the difference stays cosmetic."""
    assert {n: o for n, o in fin.SUMMARY_CODE_OVERRIDES.items() if o} == {"國泰": {"64000": "63000"}}
    assert {n: o for n, o in fin.SUMMARY_CODE_OVERRIDES_FINSUM.items() if o} == {"國泰": {"64000": "61000"}}
...
def test_P8_an_unsupported_industry_says_so_instead_of_asking_for_bank():
    ...
    msg = fin.bank_detection_message(d)
