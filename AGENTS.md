...
**這份是入口，只放地圖與紅線。** 分層、monkeypatch 目標、各套件內部的規矩，在該套件自己的 `AGENTS.md`——進去改東西時才會載入，不必一次讀完。
...
| 進入點 | 做什麼 | 套件指引 |
|---|---|---|
| `src/financialReports/` | 財報擷取。**用法定科目代碼精確比對**，不是文字比對 | [AGENTS.md](src/financialReports/AGENTS.md) |
| `src/earningsCalls/` | 法說會簡報擷取。文字比對，詞彙在 `data/con_call_terms.json` | [AGENTS.md](src/earningsCalls/AGENTS.md) |
| `src/userInteractions/cli.py` | **唯一使用者進入點**：自動分類資料夾，跑對應擷取器，合併輸出 | [AGENTS.md](src/userInteractions/AGENTS.md) |
| `src/regulatorDatasets/disclosures.py` | 金管會銀行局公開月報。**刻意完全獨立** | [AGENTS.md](src/regulatorDatasets/AGENTS.md) |
| `src/core/` | 兩個擷取器共用的解析層。**不得匯入任何擷取器** | [AGENTS.md](src/core/AGENTS.md) |

兩個擷取器都是門面：`import financialReports as fin` / `import earningsCalls as ec` 之後 `fin.X`／`ec.X` 一律有效，不必知道名字住在哪個檔。套件內部各自單向分層，圖在各自的套件指引裡。

相依只有 `openpyxl` 與 `pytest`（無 `pyproject.toml`）。建環境、CLI 用法、旗標 → [docs/SETUP.md](docs/SETUP.md)

## 動手前必讀的五條

違反這些會造成**靜默的錯誤數字**，而不是報錯。**這五條留在這裡不往下搬**——只有進到某個目錄才載入的紅線，不是紅線。
...
3. **不要合併 `financialReports` 與 `earningsCalls` 兩份同名的 `print_summary_rows` / `write_summary_csv`。** 名字相同但列的形狀不同，合併是設計決定不是清理。
...
> **測試打 monkeypatch 要打在真正被讀取的模組上**，不是套件門面。打錯地方 = 測試全綠但跑的是真程式。逐項對照表在各套件的 `AGENTS.md`。
...
長篇的來龍去脈放在 `docs/knowledge/`；程式碼裡只留**規則本身**加一行 `→ docs/knowledge/...#章節`。

判準：**這行註解消失後，會不會有人做出一個看起來完全合理的修改，而產生靜默的錯數字？**
會 → 留在程式碼旁。不會 → 進知識文件。

**依主題切，不依套件切**——檔案切法變過四次，綁在檔案上的文件會被切爛，主題不會（機構解析同時住在 `entities`、`matching`、`profitability`）。
